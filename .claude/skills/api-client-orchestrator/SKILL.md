...
description: Orchestrate repository-specific API client setup work. Use when Claude Code needs to inspect the framework and existing architecture, decide how axios, generated Swagger clients, TanStack Query, and feature-level API wrappers should fit together, delegate focused setup work, and report the final integration plan or implementation path. Use this as the top-level skill for requests like "axios instance랑 tanstack query 연결해줘", "swagger 기반 api client 구조 만들어줘", or "api 세팅해줘".
---
