...
description: Set up or align TanStack Query for the detected frontend framework. Use when Claude Code needs to determine the framework flavor first, install the correct TanStack Query package for that framework, create or update the shared QueryClient, wire the Provider into the actual app entry structure, or preserve repository-specific query defaults and error handling.
---
