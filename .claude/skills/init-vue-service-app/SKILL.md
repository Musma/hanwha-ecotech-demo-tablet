...
description: Orchestrate Vue service app initialization. Use when Claude Code needs to create a new Vue service project by running create-vue-service with verification after project creation. shadcn-vue setup is handled separately by the setup-shadcn-vue skill when requested.
---
...
```bash
node .claude/skills/init-vue-service-app/scripts/init-vue-service-app.mjs <project-name>
```

If installed outside the working repository:

```bash
node /path/to/init-vue-service-app/scripts/init-vue-service-app.mjs <project-name> --skills-root /path/to/.claude/skills
```
